...
export function getData(slug, locale, apiID, kind, preview) {
  ...
  if (kind == 'collectionType') {
    ...
    const apiUrl = `/${pluralize(
      apiID
    )}?filters[slug][$eq]=${slug}&locale=${locale}${previewParams}&populate[blocks][populate]=members.picture,header,buttons.link,faq,featuresCheck,cards,pricingCards.perks,articles,restaurants,author.picture,images,cards.image,image,background&populate=localizations&populate[seo][populate]=metaSocial.image`;
